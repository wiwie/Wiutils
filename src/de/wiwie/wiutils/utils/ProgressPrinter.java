...
public class ProgressPrinter implements ChangeListener, Serializable {
	 ...
	 */
	public static ProgressPrinter initWithValuesOf(final ProgressPrinter progress,
	 ...
	 */
	public ProgressPrinter(final long upperLimit, final boolean printOnNewPercent, final String linePrefix) {
		super();
	...
	public void update(final long newCurrent, final String optStatus) {
		...
		for (final ProgressPrinter subProgress : this.subProgress.keySet()) {
			...
			long totalNumberStepsSubProgress = this.subProgress.get(subProgress);
		...
		final float newPercent = this.currentPos > 0 ? ((float) (this.currentPos) / this.upperLimit * 100.0f) : 0.0f;
