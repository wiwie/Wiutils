...
public final class IntList implements Serializable {
	...
	public void addAll(final IntList listValues) {
		for (int i = 0; i < listValues.size(); i++) {
			this.add(listValues.get(i));
		}
	}
	...
	protected void resize(final int newsize) {
		final int[] newdata = new int[newsize]; // Create a new array
	 ...
	 * position
	 */
	public int set(final int pos, final int val) {
		final int result = this.get(pos);
		this.data[pos] = val;
		return result;
	}
