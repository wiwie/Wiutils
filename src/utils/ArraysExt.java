...
public class ArraysExt {
	...
	public static double[] average(double[][] arrays) {
		...
		for (double[] arr : arrays)
			if (arr.length > maxSize)
				maxSize = arr.length;
		return ArraysExt.average(arrays, maxSize);
	}

	public static double[] average(double[][] arrays, int targetSize) {
	...
	public static double[] hist(int min, int[] array) {
		double[] result = new double[ArraysExt.max(array) + 1 - min];
	...
	public static double[] hist(int[] array) {
		return hist(0, array);
	}
	...
	public static double max(double[][] array) {
		...
		for (double[] subArray : array)
			result = Math.max(result, subArray[maxPos(subArray)]);
		return result;
	}
	 ...
	 * @return
	 */
	public static int[] filter(int[] x, Filter largerThan) {
		List<Integer> result = new LinkedList<Integer>();
		for (int val : x)
			if (largerThan.filter(val))
				result.add(val);
